...
public class StudentPortalController {
	...
	public Map<String, List<Object>> getUserDetails(Authentication authentication) throws UnsupportedEncodingException {
		 ...
		 */
		/*
		 * model.addAttribute("attenevts", evtbyUser); model.addAttribute("listOfEvt",
		 * evt1);
		 */
	 ...
	 * 
	 * @RequestMapping(value="/getEventDetail/{id}",produces =
	 * MediaType.APPLICATION_JSON_VALUE) //@GetMapping(value =
	 * "/getEventDetail/{id}") public String getEventById(@PathVariable Long
	 * id,Model model) { System.out.println("Enterd into eventdetail"); EventDetails
	 * evt=event.getEventById(id); model.addAttribute(evt);
	 * System.out.println("Response...."+evt); return "confirmation.jsp";
	 * 
	 * }
	 */
	...
	public String registerUser(RegisterUserDetails registerDetails) {
		...
		if (registerDetails.getRefcode() != "" && registerDetails.getRefcode() != null
				&& !registerDetails.getRefcode().equalsIgnoreCase("")) {
